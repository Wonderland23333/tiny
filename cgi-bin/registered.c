...
int tableop(int n1, int n2) {
    ...
    const char *sql = "INSERT INTO stu (id, key) VALUES (?, ?)";
    ...
    if (result) {
        sprintf(content, "%sSuccess!<p>",content);
        return 1;
    } else {
        sprintf(content, "%sWrong!<p>",content);
        return 0;
    }
}
...
int main(void) {
    ...
    if (tableop(n1, n2) == 1) {
        sprintf(content, "%sRegistered Success!\r\n", content);
    }else{
        sprintf(content, "%sRegistered Falire!\r\n",content);
    }
    ...
    printf("<a href='file://./log.html'>Click here</a>");
    printf("%s", content);
